...
# DEFAULT_MODELS_DIR = './models'  # Local folder for storing models
...
def infer_tts(text,ref_clips,diffuser,diff_model,ts_model,vocoder,language=None):
    ...
    with torch.no_grad():
        ...
        mel_np = mel.squeeze().cpu().detach().numpy()  # Move to CPU if on GPU and convert to numpy

        # Generate a unique identifier using the current timestamp
        unique_id = time.strftime("%Y%m%d-%H%M%S")

        # Option 1: Save as a .npy file
        mel_file_path = os.path.join(output_folder, f'{text}.npy')
        np.save(mel_file_path, mel_np)

        # Option 2: Save as an image (spectrogram visualization)
        plt.figure(figsize=(10, 4))
        librosa.display.specshow(mel_np, x_axis='time', y_axis='mel', cmap='inferno')
        plt.colorbar(format='%+2.0f dB')
        plt.title('Mel Spectrogram')

        # Save the image as PNG
        mel_image_path = os.path.join(output_folder, f'mel_spectrogram_{unique_id}.png')
        plt.savefig(mel_image_path, bbox_inches='tight')
        plt.close()
    ...
    # Define the folder path where mel spectrograms will be stored
